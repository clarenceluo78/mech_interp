...
class HookedTransformer(HookedRootModule):
    ...
    def forward(
        ...
        stop_at_layer: Optional[int] = None,
        past_kv_cache: Optional[HookedTransformerKeyValueCache] = None,
        target_id: Optional[Union[int, List[int]]] = None,
        foil_id: Optional[Union[int, List[int]]] = None,
    ) -> Union[
        None,
        ...
        with utils.LocallyOverridenDefaults(
            ...
            if stop_at_layer is not None:
                ...
                if return_type == "logits":
                    return logits
                elif return_type == "target":
                    assert (
                        target_id is not None
                    ), "target_id must be passed in if return_type is 'target'"
                    return logits[:, -1, target_id].sum(dim=-1)
                elif return_type == "contrastive":
                    assert (
                        foil_id is not None
                    ), "foil_id must be passed in if return_type is 'contrastive'"
                    return logits[:, -1, target_id].sum(dim=-1) - logits[:, -1, foil_id].sum(dim=-1)
