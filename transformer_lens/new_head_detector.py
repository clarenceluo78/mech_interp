...
HeadName = Literal["previous_token_head", "duplicate_token_head", "induction_head", "care_nothing_head", "focus_myself_head", "specific_token_head", "neighbor_head"]
...
def detect_head(
    ...
    *,
    ...
    token_index: Optional[Tuple[List[int], List[int]]] = None,
) -> torch.Tensor:
    ...
    if isinstance(detection_pattern, str):
        ...
        if token_index is not None:
            detection_pattern = cast(
                torch.Tensor,
                eval(
                    f"get_{detection_pattern}_detection_pattern(tokens.cpu(), token_index={token_index})"
                ),
            ).to(cfg.device)
        else:
            detection_pattern = cast(
...
# focus myself head
def get_focus_myself_head_detection_pattern(
    tokens: torch.Tensor,  # [batch (1) x pos]
) -> torch.Tensor:
    """Outputs a detection score for [focus specific token heads].
       Only focus on the current token for llama.
    Args:
    ...
    # only the diagonal 1
    detection_pattern.fill_diagonal_(1)

    return torch.tril(detection_pattern)


def get_neighbor_head_detection_pattern(
    tokens: torch.Tensor,  # [batch (1) x pos]
    # m: int,
) -> torch.Tensor:
    """Outputs a detection score for nearby token heads given a margin factor.
       Focus on the current token and the next m tokens for llama.
    Args:
      tokens: Tensor being fed to the model.
      m: Margin factor indicating the number of nearby tokens to focus on.
    """
    ...
    # Fill the diagonal and the next m diagonals
    for i in range(m + 1):
        diag_indices = torch.arange(detection_pattern.shape[0] - i)
        detection_pattern[diag_indices, diag_indices + i] = 1

    return torch.tril(detection_pattern)

# care nothing head
def get_care_nothing_head_detection_pattern(
    tokens: torch.Tensor,  # [batch (1) x pos]
) -> torch.Tensor:
    """Outputs a detection score for [focus specific token heads].
       Only focus on the start token <s> for llama.
    Args:
      sequence: String being fed to the model.
    """
    
    detection_pattern = torch.zeros(tokens.shape[-1], tokens.shape[-1])
    # only the first column 1
    detection_pattern[:, 0] = 1
    print(torch.tril(detection_pattern))
    return torch.tril(detection_pattern)

# specific token head
def get_specific_token_head_detection_pattern(
    tokens: torch.Tensor,  # [batch (1) x pos]
    token_index: Union[Tuple[List[int], List[int]], List[int]],
) -> torch.Tensor:
    """Outputs a detection score for [focus specific token heads].
       Only focus on the selected indices token(s)
    Args:
      sequence: String being fed to the model.
    """
    ...
    if isinstance(token_index, tuple):
        src_token_index = token_index[0]
        tgt_token_index = token_index[1]
    else:
        tgt_token_index = token_index
        src_token_index = None
    
    # make [tgt_token_index,src_token_index] entries 1
    if src_token_index is not None:
        for src_idx in src_token_index:
            for tgt_idx in tgt_token_index:
                detection_pattern[src_idx, tgt_idx] = 1
    else:
        detection_pattern[:, tgt_token_index] = 1
