...
def get_prediction_per_layer(
    ...
    if cache is None and logits is None:
    ...
    predicted_token = logits[0, pos_idx, :].argmax(dim=-1).item()
    ...
    if target is not None:
        ...
        pred_token = predicted_token

    norm = hook_model.ln_final
    unembed = hook_model.unembed

    # Collect probabilities and ranks over layers for both target and predicted tokens
    probs = []
    ranks = []
    pred_probs = []
    pred_ranks = []
    ...
    for layer in range(hook_model.cfg.n_layers):
        ...
        # For predicted token
        pred_token_prob = prob[predicted_token].item()
        pred_token_rank = (prob > pred_token_prob).sum().cpu()
        pred_probs.append(pred_token_prob)
        pred_ranks.append(pred_token_rank)

        print(f"Layer {layer} | target prob: {target_prob:.4f} | target rank: {target_rank} | "
              f"predicted prob: {pred_token_prob:.4f} | predicted rank: {pred_token_rank}")

    if is_plot:
        import matplotlib.pyplot as plt

        # Create a figure and axis with academic-style improvements
        fig, ax1 = plt.subplots(figsize=(10, 6))

        ax1.set_xlabel("Layer", fontsize=14)
        ax1.set_ylabel("Probability", fontsize=14, color="black")
        ax1.tick_params(axis="y", labelcolor="black", colors="black")
        ax1.tick_params(axis="x", labelcolor="black", colors="black")
        ax1.grid(True, which='both', linestyle='--', linewidth=0.5)

        ax2 = ax1.twinx()
        ax2.set_ylabel("Rank", fontsize=14, color="black")
        ax2.tick_params(axis="y", labelcolor="black", colors="black")

        # Plotting the lines
        ax1.plot(probs, color="blue", linestyle='-', label='Target Token Probability', linewidth=2)
        ax2.plot(ranks, color="red", linestyle='-', label='Target Token Rank', linewidth=2)
        
        if plot_output:
            ax1.plot(pred_probs, color="green", linestyle='--', label='Predicted Token Probability', linewidth=2)
            ax2.plot(pred_ranks, color="purple", linestyle='--', label='Predicted Token Rank', linewidth=2)

        # Add a legend outside the plot area
        fig.legend(loc='upper right', bbox_to_anchor=(0.9, 0.9), fontsize=12)

        plt.title(f"Probability and Rank for '{tokenizer.decode(pred_token)}' and Predicted Token '{tokenizer.decode(predicted_token)}'", fontsize=16) \
            if plot_output else plt.title(f"Probability and Rank for Predicted Token '{tokenizer.decode(pred_token)}'", fontsize=16)
        plt.show()
