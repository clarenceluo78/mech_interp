...
HeadName = Literal["previous_token_head", "duplicate_token_head", "induction_head", "care_nothing_head", "focus_myself_head", "specific_token_head"]
...
def get_specific_token_head_detection_pattern(
    ...
    detection_pattern = torch.zeros(tokens.shape[-1], tokens.shape[-1])
    is_pair = is_list_of_tuples(token_index)

    if not is_pair and isinstance(token_index, tuple) and len(token_index) == 2:
    ...
    if src_token_index is not None:
        ...
        if is_pair:
            for src_idx, tgt_idx in token_index:
                detection_pattern[src_idx, tgt_idx] = 1
        else:
